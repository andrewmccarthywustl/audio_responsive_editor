...
def main():
    ...
    parser.add_argument("--detect-mode", default='amplitude', choices=['amplitude', 'percussive', 'harmonic'],
                        help="Method for detecting events: 'amplitude', 'percussive', or 'harmonic'.")
    ...
    parser.add_argument("--hpss-margin", type=float, default=peak_detection.DEFAULT_HPSS_MARGIN,
                         help=f"HPSS margin (default: {peak_detection.DEFAULT_HPSS_MARGIN}).")

    # Effects Argument (Updated Help Text)
    parser.add_argument("-e", "--effects", nargs='+', default=['duotone'],
                        help="List of effects to apply (e.g., duotone hue_rotate edge_color_cycle colored_edges_only). Default: duotone")
    ...
    if not os.path.exists(args.input): print(f"Error: Input video file not found: {args.input}"); return
    if args.audio and not os.path.exists(args.audio): print(f"Error: Specified audio file not found: {args.audio}"); return
    output_dir = os.path.dirname(args.output);
    if output_dir and not os.path.exists(output_dir): os.makedirs(output_dir)

    # --- Setup Temp Dirs --- (remains the same)
    temp_dir_context = tempfile.TemporaryDirectory(prefix="are_")
    if args.keep_temp: base_temp_dir = tempfile.mkdtemp(prefix="are_persistent_"); print(f"Temporary files will be kept in: {base_temp_dir}")
    else: base_temp_dir = temp_dir_context.__enter__()

    audio_file_path = None
    cuts_dir = os.path.join(base_temp_dir, "1_cuts")
    effects_dir = os.path.join(base_temp_dir, "2_effects")
    os.makedirs(cuts_dir, exist_ok=True); os.makedirs(effects_dir, exist_ok=True)
    pipeline_successful = False

    try:
        # --- Pipeline Steps --- (remains the same logic)
        print("Starting pipeline...")

        # 1. Prepare Audio & Detect Events
        ...
        if not audio_file_path: raise RuntimeError("Failed to prepare audio file.")
        event_times = peak_detection.find_peaks(
            audio_path=audio_file_path, mode=args.detect_mode, target_peaks=args.target_cuts,
            min_gap_seconds=args.min_gap, hpss_margin=args.hpss_margin
        )
        ...
        if not cutter.cut_video_at_times(args.input, event_times, cuts_dir): raise RuntimeError("Failed during video cutting stage.")
        ...
        print(f"\n--- Pipeline Error ---"); print(f"Error: {e}"); print("Traceback:"); traceback.print_exc(); print("--- End Traceback ---")
        ...
        if not args.keep_temp:
            if 'temp_dir_context' in locals() and base_temp_dir.startswith(tempfile.gettempdir()):
                ...
                try: temp_dir_context.__exit__(None, None, None)
                except Exception as cleanup_e: print(f"Warning: Error during automatic temp directory cleanup: {cleanup_e}")
            elif 'base_temp_dir' in locals() and not base_temp_dir.startswith(tempfile.gettempdir()): pass
        elif pipeline_successful: print(f"Temporary files kept at: {base_temp_dir}")
        else:
             if 'base_temp_dir' in locals(): print(f"Pipeline failed. Temporary files kept for debugging at: {base_temp_dir}")
